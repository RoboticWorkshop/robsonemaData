...
class Server:
	def __init__(self):
		...
		self.lamda = np.matrix([[25., 0., 0.], [0., 25., 0.], [0., 0., 25.]])	#saat posisi masih jauh dengan posisi target
		self.lamda1 = np.matrix([[48., 0., 0.], [0., 48., 0.], [0., 0., 50.]])  #saat posisi sudah dekat dengan posisi target
		self.lamda2 = np.matrix([[58., 0., 0.], [0., 58., 0.], [0., 0., 60.]])	#saat akan menendang
		self.kp = np.matrix([[0., 0., 0.], [0., 0., 0.], [0., 0., 0.]])
		#self.visual_lamda = np.matrix([[30., 0., 0.], [0., 42., 0.], [0., 0., 42.]])
		self.visual_lamda = np.matrix([[40., 0., 0.], [0., 45., 0.], [0., 0., 45.]])		#saat bola masih jauh
		self.visual_lamda1 = np.matrix([[50., 0., 0.], [0., 55., 0.], [0., 0., 55.]])		#saat bola sudah dekat
		#lambda untuk mengambil bola mati
		self.visual_lamda2 = np.matrix([[25., 0., 0.], [0., 25., 0.], [0., 0., 28.]])		#saat bola masih jauh
		self.visual_lamda3 = np.matrix([[40., 0., 0.], [0., 40., 0.], [0., 0., 43.]])		#saat bola sudah dekat		
		self.ts = 0.1
		...
		self.ball_des1 = np.matrix([[0.45, 0., 0.]]).T	#desired saat mendekati bola mati
		#=======================================================================================
		self.game_mode = 0.
		self.attack = 0.
		self.treshold_error = 0.25 									#threshold for position error
		self.treshold_error_kickoff = 0.125 						#threshold for position error
		self.treshold_error_shooting = 0.325 						#threshold for position error
		...
		self.limit_pwm = 500										#maximum RPM/PWM
		...
		self.corner_pos = 0
		#game variables
		self.strategy = 0	#strategy permainan yang diberikan node manager
		self.pass_ball = 0	#indikator kondisi bola mati
		self.pass_ball_lock = 0 #berhenti sebelum dan sesudah melakukan umpan
		self.robot2_ball_flag = 0 #indikator bola pada robot 2
		self.robot1_ball_flag = 0 #indikator bola pada robot 1
		self.lock = 0 # merubah gerakan mengejar robot, 0 = menggunakan visual invers kinematic, 1 = mendekati bola secara langsung
		self.dribbler_lock = 0 #berfungsi untuk mematikan dan menyalakan dribbler, 1 = mati / pada saat positioning dan pada saat bola masih jauh dari robot, 0 = dribbler menyala
		self.kick_ball = 0 #variabel penendang bola
		self.get_ball_secs = 0 #detik untuk mengatur perubahan kecepatan dribbler bola ketika robot berhenti
		self.lost_ball_secs = 0 #detik ketika robot kehilangan pandangan terhadap bola, mendelay robot untuk berhenti
		self.loss_ball_secs = 0 #detik ketika robot menggiring bola, digunakan untuk melakukan perintah update posisi ketika robot mendapatkan bola dalam waktu kehilangan bola yang sudah ditentukan
		self.stop_ball = 0	#detik yang digunakan untuk menghentikan robot ketika baru mendapatkan bola, ketika batas waktu telah dilewati maka robot akan bergerak menuju posisi target menembak	
		self.kick_ball_delay = 0 #detik untuk menunda melakukan umpan dan melakukan shooting ketika target telah ditemukan
	...
	def pass_ball_status_callback(self, dat):
		if(dat.data == 1):
			self.pass_ball = 0
	...
	def pose_des_callback(self, dat):		
		...
		if(bearing >= 0):
			self.tj_ang = bearing - np.radians(180.)
		else:
			self.tj_ang = bearing + np.radians(180.)
		...
		if(self.dribbler_lock == 1) or (self.robot1_ball_flag != 0):
			...
			if(e_norm < 0.75):
				...
				if(e_norm < 1.0):
					...
					self.kp = self.lamda
				if(self.pass_ball != 1):
					if(self.pose[2,0]>np.radians(90)) and (self.tj_ang < np.radians(-90)):
						error[2,0] = 180 + (self.tj_ang % np.radians(180))
					elif(self.pose[2,0]<np.radians(-90)) and (self.tj_ang > np.radians(90))
						error[2,0] = -180 + (self.tj_ang % np.radians(-180))
					else:
						error[2,0] = self.tj_ang - self.pose[2,0]
			#print error[2,0]
			...
			#dribbler.y = 180
			if(e_norm_shoot > tresh):
				if(ers[0,0]<-1.0)or(abs(ers[1,0])>1.0):
					dribbler.x = 200
					dribbler.y = 200
				else:
					dribbler.x = 150
	...
	def visual_invers_kinematic(self): #ketika mengambil bola mati
		Jinv = np.linalg.inv(self.Jr)
		if(self.strategy == 2): #Strategy ketika bola lebih dekat dengan robot2
			...
			if(self.lock==1):
				...
				if(self.ball_error[2,0] <= 0.15 and self.ball_error[2,0] >= -0.08):
					w[0,0] = 0.   
					w[1,0] = 300.
					w[2,0] = -300.
				elif(self.ball_error[2,0] > 0.15):
					w[0,0] = 0.
					w[1,0] = 300 + (abs(self.ball_error[2,0])*100)
					w[2,0] = -300 + (abs(self.ball_error[2,0])*100)
				elif(self.ball_error[2,0] < -0.08):
					w[0,0] = 0.
					w[1,0] = 300 - (abs(self.ball_error[2,0])*100)
					w[2,0] = -300 - (abs(self.ball_error[2,0])*100)
	...
	def visual_invers_kinematic1(self): #ketika mengejar bola pada permainan
		Jinv = np.linalg.inv(self.Jr)
		if(self.strategy == 2): #Strategy ketika bola lebih dekat dengan robot1
			#digunakan ketika robot kehilangan bola
			#robot tidak langsung berhenti, tapi tetap bergerak ke arah dimana bola terdeteksi untuk beberapa saat
			if(np.linalg.norm(self.ball_pos) == 0):
				self.lost_ball_secs += 1
			...
			if(self.lock==1):
				...
				if(self.ball_error[2,0] <= 0.15 and self.ball_error[2,0] >= -0.08):
					...
					if(self.ball_pos[0,0] <= 0.4):
						w[0,0] = 0.
						w[1,0] = 300 + (abs(self.ball_error[2,0])*100)
						w[2,0] = -300 + (0.5*abs(self.ball_error[2,0])*100)
					else:
						w[0,0] = 0.
						w[1,0] = 350 + (abs(self.ball_error[2,0])*100)
						w[2,0] = -350 + (0.3*abs(self.ball_error[2,0])*100)
				elif(self.ball_error[2,0] < -0.08):
					if(self.ball_pos[0,0] <= 0.4):
						w[0,0] = 0.
						w[1,0] = 300 - (0.5*abs(self.ball_error[2,0])*100)
						w[2,0] = -300 - (abs(self.ball_error[2,0])*100)
					else:
						w[0,0] = 0.
						w[1,0] = 350 - (0.3*abs(self.ball_error[2,0])*100)
						w[2,0] = -350 - (abs(self.ball_error[2,0])*100)
			else:
				w = gain*Jinv*self.ball_error				
		else:
			self.ball_error = self.ball_error * 0
	...
	def main(self):
		if(self.game_mode == 0):
			...
			if(self.pass_ball_lock == 0):
				if(self.robot1_ball_flag != 0) or (self.robot2_ball_flag != 0) or (self.strategy == 1):
					if(self.robot2_ball_flag == 1) and (self.stop_ball < 10):					
						self.stop_motion()
						print "STOP A WHILE ..."
					else:
					...
					self.stop_ball = 0
				if(self.kick_ball == 1):
					if(self.kick_ball_delay > 20):
						if(self.pass_ball == 1):
							#pass ball command						
							if(self.robot2_ball_flag == 0):
								self.pass_ball = 0
								self.pass_ball_lock = 1
						...
						print "AIM FOR TARGET!!!"
					self.kick_ball_delay += 1
				else:
					self.kick_ball_delay = 0
			else:
				self.stop_motor()
				if(self.stop_ball > 50):
					self.pass_ball_lock = 0
				self.stop_ball += 1			
...
if __name__ == "__main__":
	...
	command_update = rospy.Publisher("robot2/command_update", Int32, queue_size = 1)
	server = Server()
	motor = Vector3()
	dribbler = Vector3()
	try:
		...
		rospy.Subscriber("/team/pass_complete_flag", Int32, server.pass_ball_status_callback)
		rospy.spin()
