...
class Server:
	def __init__(self):
		...
		self.lamda1 = np.matrix([[40., 0., 0.], [0., 40., 0.], [0., 0., 42.]])
		self.lamda2 = np.matrix([[35., 0., 0.], [0., 35., 0.], [0., 0., 37.]])
		...
		self.visual_lamda3 = np.matrix([[35., 0., 0.], [0., 35., 0.], [0., 0., 38.]])		
		self.ts = 0.1
		...
		self.treshold_error_shooting = 0.35 							#threshold for position error
	...
	def pose_callback(self, dat):
		...
		if(self.game_mode == 0) or (self.game_mode == 2):
			self.main()
	...
	def ball_flag_callback(self, dat):		
		if(dat.data != self.robot1_ball_flag):
			self.get_ball_secs = 0
	...
	def obstacle_avoidance_kinematic(self):
		...
		if(self.dribbler_lock == 1) or (self.robot2_ball_flag != 0):
			if(e_norm < 1.0):
				self.kp = self.lamda1
			else:
				self.kp = self.lamda		
	...
	def visual_invers_kinematic(self):
		Jinv = np.linalg.inv(self.Jr)
		if(self.strategy == 1): #Strategy ketika bola lebih dekat dengan robot2
			...
			if(self.lock==1):
				...
				if(self.ball_error[2,0] <= 0.15 and self.ball_error[2,0] >= -0.05):
					w[0,0] = 0.   
					w[1,0] = 300.
					w[2,0] = -300.
	...
	def visual_invers_kinematic1(self):
		Jinv = np.linalg.inv(self.Jr)
		if(self.strategy == 1): #Strategy ketika bola lebih dekat dengan robot1
			#digunakan ketika robot kehilangan bola
			#robot tidak langsung berhenti, tapi tetap bergerak ke arah dimana bola terdeteksi untuk beberapa saat
			if(np.linalg.norm(self.ball_pos) == 0):
				self.lost_ball_secs += 1
	...
	def main(self):
		if(self.game_mode == 0):
			...
			if(self.robot1_ball_flag != 0) or (self.robot2_ball_flag != 0) or (self.strategy == 2):
				if(self.robot1_ball_flag == 1) and (self.stop_ball < 15):					
					self.stop_motion()
					print "STOP A WHILE ..."
				else:
...
if __name__ == "__main__":
	...
	dribbler_publisher = rospy.Publisher("robot1/dribbler", Vector3, queue_size = 3)
	...
	try:
		...
		rospy.Subscriber("/robot1/ball_flag", Int32, server.ball_flag_callback)
		rospy.Subscriber("/robot2/ball_flag", Int32, server.robot1_ball_flag_callback)
